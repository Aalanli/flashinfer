...
template <int K_LDG_STAGES, int K_LDG_SCALES_STAGES, int K_MMA_STAGES, int EPILOGUE_WARPGRPS,
          int VECTORIZE_TCGEN_LD, int PRELOAD_TCGEN_REGS,
          int Q_NEXT>
    ...
    for (int idx = 0; idx < divup(num_pages, EPILOGUE_WARPGRPS * 2); idx++) {
      ...
      for (int q_next = 0; q_next < Q_NEXT; q_next++) {
        ...
        if (offset < seq_end) {
          ...
          out_logits[q_next * max_num_pages * 64 + offset] = final_sum * k_seq_scale;
        }
      }
...
template <int K_LDG_STAGES, int K_LDG_SCALES_STAGES, int K_MMA_STAGES, int EPILOGUE_WARPGRPS,
          int VECTORIZE_TCGEN_LD, int PRELOAD_TCGEN_REGS, int Q_NEXT>
__global__ __launch_bounds__(EPILOGUE_WARPGRPS * 7 * 32, 1) void mqa_v2_kernel_small_batch(
  ...
  if (num_pages > 0) {
    ...
    mqa_v2_kernel<K_LDG_STAGES, K_LDG_SCALES_STAGES, K_MMA_STAGES, EPILOGUE_WARPGRPS,
...
extern "C" void launch_mqa_logits(uint8_t* q_ptr, uint8_t* k_ptr, float* weights, int* seq_lens,
  ...
  setup_kernel_smem_once<
      mqa_v2_kernel_small_batch<K_LDG_STAGES, K_LDG_SCALES_STAGES, K_MMA_STAGES, EPILOGUE_WARPGRPS,
                                VECTORIZE_TCGEN_LD, PRELOAD_TCGEN_REGS, Q_NEXT>,
      131 * 1000>();
  ...
  mqa_v2_kernel_small_batch<K_LDG_STAGES, K_LDG_SCALES_STAGES, K_MMA_STAGES, EPILOGUE_WARPGRPS,
      ...
      <<<sm_multiple * num_sms, THREADS, smem_bytes, stream>>>(
          ...
          sm_multiple, logits, logits_stride);
}
