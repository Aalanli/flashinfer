...
template <typename T> __device__ inline int cvt_addr(T *addr) {
...
// this kernel assumes that max_num_pages is divisible by 2

// one extra to pad to a whole warp group

template <int K_LDG_STAGES, int K_LDG_SCALES_STAGES, int K_MMA_STAGES,
          int EPILOGUE_WARPGRPS, int VECTORIZE_TCGEN_LD, int PRELOAD_TCGEN_REGS,
          int Q_NEXT>
    ...
    for (int idx = 0; idx < divup(num_pages, EPILOGUE_WARPGRPS * 2); idx++) {
      ...
      for (int q_next = 0; q_next < Q_NEXT; q_next++) {
        ...
        if (offset < seq_end) {
          ...
          out_logits[q_next * max_num_pages * 64 + offset] =
              final_sum * k_seq_scale;
        }
      }
...
template <int K_LDG_STAGES, int K_LDG_SCALES_STAGES, int K_MMA_STAGES,
          ...
          int Q_NEXT>
__global__
__launch_bounds__(EPILOGUE_WARPGRPS * 7 * 32, 1) void mqa_v2_kernel_small_batch(
    ...
    float *__restrict__ out_logits, // [BatchSize, max_seq_len]
    const int logits_stride) {

  int sm_id = blockIdx.x / sm_multiple;
  int sm_loc = blockIdx.x % sm_multiple;
  __shared__ int4 sm_map_shared;

  if (threadIdx.x == 0) {
    sm_map_shared = sm_mapping[sm_id];
  }
  __syncthreads();
  int4 sm_map = sm_map_shared;
  ...
  if (num_pages > 0) {
    ...
    const int stride = logits_stride;

    mqa_v2_kernel<K_LDG_STAGES, K_LDG_SCALES_STAGES, K_MMA_STAGES,
...
extern "C" void launch_mqa_logits(uint8_t *q_ptr, uint8_t *k_ptr,
                                  float *weights, int *seq_lens,
                                  int *block_table, float *logits, int4 *sm_map,
                                  int max_num_pages, int num_pages,
                                  int batch_size, int logits_stride,
                                  int num_sms, int sm_multiple,
                                  cudaStream_t stream) {
  ...
  setup_kernel_smem_once<
      mqa_v2_kernel_small_batch<K_LDG_STAGES, K_LDG_SCALES_STAGES, K_MMA_STAGES,
                                EPILOGUE_WARPGRPS, VECTORIZE_TCGEN_LD,
                                PRELOAD_TCGEN_REGS, Q_NEXT>,
      131 * 1000>();
  ...
  mqa_v2_kernel_small_batch<K_LDG_STAGES, K_LDG_SCALES_STAGES, K_MMA_STAGES,
      ...
      <<<sm_multiple * num_sms, THREADS, smem_bytes, stream>>>(
          ...
          max_num_pages, num_pages, sm_multiple, logits, logits_stride);
}
