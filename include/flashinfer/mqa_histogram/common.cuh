...
__device__ inline int ld_acquire(const int32_t *__restrict__ addr) {
  int res;
  asm volatile("ld.acquire.cta.b32 %0, [%1];"
               : "=r"(res)
               : "l"(addr)
               : "memory");
  return res;
...
// topk kernel v2
template <typename T> __host__ __device__ __forceinline__ T divup(T a, T b) {
  return (a + b - 1) / b;
}
...
__device__ __forceinline__ void WLMS(uint8_t val, int *shared_hist,
...
__device__ __forceinline__ void reduce_shared(int *addr, int val) {
  asm("red.relaxed.cta.shared::cta.add.s32 [%0], %1;" ::"l"(addr), "r"(val)
      : "memory");
}
template <auto *kernel_func, size_t smem_bytes> void setup_kernel_smem_once() {
...
template <auto *kernel_func> void setup_non_portable_clusters_once() {
  static const cudaError_t result = []() -> cudaError_t {
    auto func_ptr = kernel_func;
    return cudaFuncSetAttribute(
        func_ptr, cudaFuncAttributeNonPortableClusterSizeAllowed, 1);
  }();
...
__device__ inline void mbarrier_cpy(int *src, int *dst, int num_bytes,
                                    uint64_t *mbar) {
